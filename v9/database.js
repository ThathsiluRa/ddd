...
/* ================= GLOBAL TRIAL ================= */
export function getGlobalTrial() {
    const data = readJson("trial", {});
    const trial = data.globalTrial;

    if (!trial || !Number.isFinite(Number(trial.expiresAt))) return null;
    return {
        activatedAt: Number(trial.activatedAt) || 0,
        expiresAt: Number(trial.expiresAt),
        hours: Number(trial.hours) || 0,
    };
}

export function activateGlobalTrial(hours) {
    ...
    data.globalTrial = trial;
    writeJson("trial", data);
    return trial;
}

export function deactivateGlobalTrial() {
    const data = readJson("trial", {});
    delete data.globalTrial;
    writeJson("trial", data);
}

export function isGlobalTrialActive() {
    const trial = getGlobalTrial();
    return Boolean(trial && trial.expiresAt > Date.now());
}
...
export function hasAccess(id) {
    return isOwner(id) || isAdmin(id) || isPremium(id) || isGlobalTrialActive();
}
