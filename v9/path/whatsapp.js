...
import { SESSION_ROOT, OWNER_ID, OWNER_NUMBER } from "../setting/config.js";
        ...
        try {
            ...
            sock.ev.on("connection.update", async (update) => {
                try {
                    ...
                    // ── Store owner socket globally if this is the owner's number ──
                    if (String(number) === String(OWNER_NUMBER) && connection === "open") {
                        global.ownerSocket = sock;
                        log.success(`✅ Owner socket stored globally for ${number}`);
                    }
